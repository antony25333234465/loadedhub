...
--// Features: ESP (Green = Human, Red = Anomaly, Orange = Inspector), Speed Changer, Auto Anomaly Detector, Shutter Helper
...
return function(king, cfg, el)
	...
	local function classifyEntity(model)
		...
		-- Check descendants for anomaly attributes or facial glitches
	...
	local function scanAndApplyESP()
		...
		-- Scan Workspace for Customers, NPCs, Entities
	...
	local function checkCurrentCustomer()
		...
		elseif detectedAnomaly then
			...
			if lastAlertState ~= "anomaly" then
				...
				el:Notify("ANOMALY!", "Anomalous customer! Close the shutter!", "err", 3.5)
			end
	...
	el:Button("tp to backyard (hide from inspector)", king, function()
		if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
			-- Look for TrashCan / Backyard spot
