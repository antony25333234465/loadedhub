...
--// 100% Accurate Body Outline & Fill ESP (GREEN = Normal Human / RED = Anomaly)
...
return function(king, cfg, el)
	...
	-- COLORS
	----------------------------------------------------------
	local GREEN = Color3.fromRGB(0, 255, 100)   -- Normal Human (Safe - Green Fill & Outline)
	local RED   = Color3.fromRGB(255, 30, 40)   -- Anomaly (Unsafe - Red Fill & Outline)
	...
	local function checkIsAnomaly(model)
		...
		-- 1. Check Attributes on the model
		...
		-- 3. Check Descendants for Anomaly Values / Flags
		for _, v in ipairs(model:GetDescendants()) do
			...
			-- Check blank face / missing face decal
	...
	local function isCustomerModel(model)
		...
		-- Must have Humanoid or Head or Customer/NPC in name
	...
	----------------------------------------------------------
	-- BODY OUTLINE & FILL ESP ENGINE (GREEN = HUMAN / RED = ANOMALY)
	----------------------------------------------------------
	...
	local function applyBodyESP(model, isAnomalous)
		...
		-- Create Full Body Outline & Fill Highlight
		...
		hl.FillColor = targetColor
		hl.OutlineColor = targetColor
		hl.FillTransparency = 0.40  -- Body Fill
		hl.OutlineTransparency = 0.0 -- Solid Outline Around Body
	...
	local function updateESPAndStatus()
		...
		-- Scan Workspace for Customer Models
		for _, obj in ipairs(ws:GetDescendants()) do
			if obj:IsA("Model") and isCustomerModel(obj) then
				...
				if anomalous then
					foundAnomalyAtCounter = true
				else
					foundHumanAtCounter = true
				end
			end
		end
	...
	-- ESP Heartbeat Thread
	task.spawn(function()
		while true do
			if espEnabled then
				pcall(updateESPAndStatus)
			end
			task.wait(0.3)
		end
	end)
	...
	el:Button("toggle shutter (close/open)", king, function()
		...
		if toggled then
			el:Notify("shutter", "toggled shutter", "ok", 1.8)
		else
